...
log = require('log4js').getLogger("cradle");
...
cradle.Connection.prototype.request = function (method, path, /* [options], [data], [headers] */ callback) {
    ...
    // Ensure socket errors are informed to the caller.
    ...
    request.on('response', function (res) {
        ...
        res.on('data', function (chunk) {
            ...
            // Ensure response errors are informed to the caller
            callback(err, null);            
        });
    });
};
    ...
    return {
       ...
       _merge: function (id, doc, callback) {
        ...
        list: function (path, options) {
            var args = new(Args)(arguments), keys;
            path = path.split('/');
            path = ['_design', path[0], '_list', path[1], path[2]].join('/');
        ...
        changes: function (options, callback) {
		    ...
		    var chunk = "", response = new(events.EventEmitter);
                    res.setEncoding('utf8');
		    ...
		    response.originalResponse = res;
			       ...
			       chunk += data;
			       if (chunk.indexOf("\n") > -1) {
				   var part = chunk.substring(0, chunk.indexOf("\n")).trim();
				   if (part) {
				       response.emit('data', JSON.parse(chunk.substring(0, chunk.indexOf("\n"))));
				   }
